...
class Game(ttk.Frame):
    ...
    def get_path(self, file_kind: str, file: str) -> str:
        """Return the path to a file. Works however the program is run, through an .app file or the debugger

        :param file_kind: The kind of file, either 'image' or 'sound'
        :param file: The name of the file
        """
        return os.path.join(getattr(settings, file_kind + 'Dir'), file)
    ...
    def build_main_ui(self):
        ...
        # THE INFO AREA
        # The thin line
        thin_line_img = Game.register_image(PhotoImage(Image.open(self.get_path('image', 'line.png'))))

        # Place the thick line
        self.mainCanvas.create_image(2, 240, image=Game.register_image(PhotoImage(Image.open(
                                    self.get_path('image', 'thick_line.png')))),
                                    anchor=tk.W)
        ...
        clock = self.mainCanvas.create_image(thin_line_img.width() + 80, style.infoLabelY,
                                             image=Game.register_image(PhotoImage(Image.open(
                                                      self.get_path('image', 'clock_low_shadow.png')))),
                                             anchor=tk.N+tk.W)
        ...
        button_images = [Game.register_image(PhotoImage(Image.open(
            self.get_path('image', button + '_button.png')))) for button
    ...
    def build_settings_ui(self):
        ...
        settings_main_canvas.create_image(style.mainWindowMinWidth - style.mainWindowMinWidth // 5,
                                          ...
                                          image=Game.register_image(PhotoImage(Image.open(
                                            self.get_path('image', 'cog_wheel.png')))))
    ...
    def signal_word_guessed(self, sound: str = 'word_correct.wav'):
    ...
    def load_words(self, words_file: str) -> dict:
        ...
        try:
            ...
            sys.exit('The game\'s words database is not accessible. Make sure the \'word.json\' '
                     'file is where it\'s supposed to be. Terminating.')
        finally:
